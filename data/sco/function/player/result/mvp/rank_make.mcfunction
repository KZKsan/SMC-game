...
execute as @a[predicate=sco:team_join,team=!spectator] run scoreboard players operation #MVP_hit_arrow result_mvp > @s score_hit_arrows.score_kill_count.total
execute as @a[predicate=sco:team_join,team=!spectator] run scoreboard players operation #MVP_damage result_mvp > @s score_damage_dealt.score_kill_count.total
