...
#プロセスID
execute if score #runtime game matches 1 run scoreboard players set @a[predicate=sco:team_join] processID 211
#切断処理
...
execute if score #runtime game matches 4.. as @e[type=item_display,tag=beacon.core,tag=load_pos] run function sco:process/domination/object/beacon/set/load_pos with entity @s item.components."minecraft:custom_data"
#execute if score #runtime game matches 4.. 

#プロセス終了
execute if score #mst_time timer matches -110 run scoreboard players set #process game 220
