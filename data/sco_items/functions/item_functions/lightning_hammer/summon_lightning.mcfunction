...
execute store success score # _ if score # _ matches 0 if entity @s[team=mode.practice] at @e[sort=random,type=creeper,tag=sandbag,limit=1,distance=..20] summon lightning_bolt run function sco_items:item_functions/lightning_hammer/particle
